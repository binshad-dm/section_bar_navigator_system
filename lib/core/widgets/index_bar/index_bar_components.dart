...
class ScrollHintSelectorState extends State<ScrollHintSelector> {
  ...
  void _onScroll() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      cubit.setCurrentScrollPosition(
        cubit.controller?.offset,
      );
    });
    showWidgetOnScroll();
  }
  ...
  Future<void> _snapToNearestTag(double containerHeight) async {
    ...
    if (context != null) {
      ...
      final double targetOffset =
          context.read<SettingsCubit>().controller!.offset + (itemCenter - listCenter);
  ...
  Widget build(BuildContext context) {
    ...
    return Visibility(
      ...
      child: Align(
            alignment: context.watch<SettingsCubit>().handnessType == HandnessType.right
                      ...
                      if (_currentSelectedName != null)
                        if (context.watch<SettingsCubit>().handnessType == HandnessType.right)
                          Positioned(
                            ...
                            top:
                                (containerHeight / 2) -
                                _calculateHintHalfHeight(),
                            child: CustomHintWidget(
                              opacity: _overlayOpacity,
                              text: _currentSelectedName!,
                              color: widget
                                  .listData[currentSelectedIndex]
                                  .tabColor,
                            ),
                          ),
                      if (context.watch<SettingsCubit>().handnessType == HandnessType.left)
                        Positioned(
                          ...
                          child: CustomHintWidget(
                            ...
                            text: _currentSelectedName!,
                            color:
                                widget.listData[currentSelectedIndex].tabColor,
                          ),
                        ),
                    ],
                  ),
